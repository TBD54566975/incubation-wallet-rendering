...
class VerifiableCredential extends HTMLElement {
  ...
  set manifest(manifest) {
    ...
    console.log("--------- checking object", manifest !== undefined);
    ...
    console.log("----------- manifest before ", this.#manifest);
    this.#update();
    console.log("done set manifest");
  }
  ...
  attributeChangedCallback(name, oldValue, newValue) {
    switch (name) {
      ...
      case "manifest":
        ...
        console.log("fetching new value", newValue);
  ...
  #update() {
    ...
    if (this.#data["spec_version"] !== VERSION) return;

    // <https://identity.foundation/credential-manifest/#output-descriptor>
    let descriptors =
      verifyType(this.#data["output_descriptors"], Array.isArray) ?? [];
...
(function (serverside, global, $, _, JSON) {
  ...
  $.fn.credentialRender = function (options) {
    ...
    vc.manifest =
      "https://identity.foundation/credential-manifest/schemas/credential-manifest.json";
    vc.data = options.data;
    console.log(vc);
    domEl.html(vc);
  };
